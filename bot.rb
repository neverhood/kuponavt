...
$categories = {
  'Кафе, рестораны' => [ %w(мексиканск+блюд country+chicken баскин чайкоф кофе+хауз романт+свидан романт+ужин чайн+церемон кафе ресторан меню _чай+_бар чайн+клуб шедевр+кухн), %w(суши пицца техн+кухн кафел стрип караоке _мойк+_авто _катани бильярд боулинг тренинг _spa_ _спа_ массаж+_чай клуб+бар бабочк американо эспрессо _кофейня _кофейне_) ],
  'Пицца' => [ %w(пицц) ],
  'Суши' => [ %w(суши_ ролл японск+кухн _вкус+_япони), %w(мезороллер магазин япокнск+кухня _ролик) ],
  ...
  'Концерты, цирк' => [ %w(баскетбольн+матч концерт шоу цирк представлен _stand+_up_ фестивал), %w(клуб театр _дом шоу+студ доставк шоу+_рум выставк _отел представленн _lego герой+мультфильм съемка видемонтаж представление+_о_ циркон занятия _одежд+шоу цирк+_курс) ],
  'Боулинг, бильярд' => [ %w(боулинг бильярд), %w(_баз+отдых романтич+выходн проживан керлинг _саун _мойк+_авто) ],
  'Театры, музеи, выставки' => [ %w(героическ+комед театр мюзикл спектакл _музе выставк хор_), %w(курс актерск+мастерств кино светодиод+шоу занят+театр+студ) ],
  'Пейнтбол, лазертаг' => [ %w(пейнтбол лазертаг лазрен+тир), %w(перелет) ],
  'Картинг' => [ %w(картинг заезд), %w(снегоход перелет) ],
  'Аквапарки' => [ %w(аквапарк) ],
  'Активный отдых' => [ %w(катани+ролик уницикл аттракцион zorb зорб сноуборд верхов+езд кайтинг конюшн скалодром скалолаз батут фототриал снегоход квадроцикл _гольф воздушн+шаре экстрим лабиринт конн+прогулк колес+обозрен дайвинг каток катка _ката+коньк прокат+коньк), %w(лабиринт+волос романт+отдых проживан одежд магазин гольфы гольфов гольфин фотосесси) ],
  'Парикмахерские' => [ %w(феном_ преческ плетен+_кос волос стрижек стрижк укладк ламинирован мелирован каутеризац наращиван+волос завивк выпрямлен окрашиван биозавивк восстановлен+волос косичк), %w(наращиван+ресниц завивк+ресниц мезотерап набор аксессуар маникюр+наращиван наращиван+ногт эпиляц средств+рост сыворотк шампун собак _кошк фотосесси машинк+волос паркет ламинир+ногт стилист+макияж плойк+для+волос щипц+для+волос интернет+магазин продукт+для+волос расческ+для+волос удален+волос депиляц маникюр+стрижк маникюр+волос собак _зоо обучен _керами+выпрямител) ],
  'Солярии' => [ %w(минут+загара соляр моментальн+загар), %w(тренажер+_зал фитнес+клуб аэроб _танц _заняти маникюр солярис) ],
  ...
  'SPA салоны' => [ %w(stone+массаж саун+массаж+cgf саун+массаж+spa фитосаун фитобочк кедров+бочк spa _спа_ обертыван _обёртыван антицеллюлитн пенн+массаж), %w(машин авто спальн экскурси _спайк тур _спа+отел обучени маникюр собак _зоо спайси местн+номер) ],
  ...
  'Компьютерная помощь' => [ %w(компьютер+сервис компьютер+помо ремонт+компьютер обслуживан+_пк_) ],
  ...
  'Ремонт и уборка в доме, дизайн' => [ %w(наливн+ванн натяжн+потол утановк+окон пластик+окна установк+вентиляц установк+счетч прачечн пластик+окн квартир ремонт+кварт чистк+мебел уборк диз+кварт пластик+окна _ковр переезд дизайн+кухон дизайн+проект дизайн+комн дизайн+интерьер чистк+мягк+мебел чискт+_ковр), %w(_авто пылесос) ],
  ...
  'Для животных' => [ %w(_попуга зоо+такси _кошк _корм собак pedigree кастриров кошачь cat) ],
  'Подарки, игрушки, интересные штуки' => [ %w(air+swimmer _lego_ радиоуправл светодиод ночник фотоподарк _бонг небесн+фонар bobblehead фотопазл подарочн подар _игр покер вертол), %w(подар+здоров) ],
}
