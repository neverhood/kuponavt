...
$categories = {
  'Кафе, рестораны' => [ %w(кафе ресторан меню кухн), %w( суши пицца техн+кухн ) ],
  'Бары, клубы, караоке' => [ %w(бар клуб party дискотек вечеринк караоке), %w(стоматолог дайвинг) ],
  'Пицца' => [ %w(пицца) ],
  'Суши' => [ %w(суши ролл) ],
  ...
  'Товары для красоты' => [ %w(pupa косметик крем аромат парфюм лак+волос Schwarzkopf _тен _кист _туш maybelline набор _мыл бритв _дух шампун туалет+вод), %w(чай+аромат пилинг+космет чистка авто+косметика набор+нож набор+карт) ],
  'Товары для дома' => [ %w(мебел стол подставк пуф кресл кресел постельн техн+кухн дом шкаф чашк одеял подушк покрывал воздух жалюз быт+техн будильн ночник диван пластик+окна сервиз термос настен+час), %w(стол+игр) ],
  'Электроника' => [ %w(планшет apple ipod ipad компьют psp дюйм  mp4 плеер наушник навигатор мышк _мыш видеорегистр электрошок touch электрон беспроводн гарнитур wi+fi интернет вертол электротовар iphone телевизор диагонал ноутбук нетбук телефон устройств модем камер), %w(подставк стол) ],
