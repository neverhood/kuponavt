...
$categories = {
  ...
  'Театры, музеи, выставки' => [ %w(репертуар сатирикон лирическ+комед героическ+комед театр мюзикл спектакл _музе выставк хор_), %w(курс актерск+мастерств кино светодиод+шоу занят+театр+студ) ],
  'Пейнтбол, лазертаг' => [ %w(пейнтбол лазертаг лазрен+тир), %w(перелет) ],
  'Картинг' => [ %w(картинг заезд), %w(снегоход перелет) ],
  'Аквапарки' => [ %w(аквапарк) ],
  'Активный отдых' => [ %w(_каток_ _катке_ катани+ролик уницикл аттракцион zorb зорб сноуборд верхов+езд кайтинг конюшн скалодром скалолаз батут фототриал снегоход квадроцикл _гольф воздушн+шаре экстрим лабиринт конн+прогулк колес+обозрен дайвинг каток катка _ката+коньк прокат+коньк), %w(лабиринт+волос романт+отдых проживан одежд магазин гольфы гольфов гольфин фотосесси) ],
  ...
  'Для бизнеса' => [ %w(unisender франчайзинг фото+виртуальн+тур контекстн+реклам создан+сайт констру+сайт логотип бухгалтер _1с предприят сайт+визитк отчетност юридическ ооо _ип грузоперевозк бизнес хостинг документ правов лиценз сайт+ключ визитк) ],
  ...
  'Организация праздников' => [ %w(проведен+праздник _юбил корпоратив свадьб свадебн) ],
  'Другие услуги' => [ %w(астролог перевод фото+пазл оцифров+видео видеомонтаж детектив земельн+участ календар+фото фото+_аур кружк+фото оцифров+видео прокат+наряд реферат диплом дипломн+работ строительств энергетическ+пол диагност+аур _гадани) ],
  'Для животных' => [ %w(_попуга зоо+такси _кошк _корм собак pedigree кастриров кошачь cat) ],
  'Подарки, игрушки, интересные штуки' => [ %w(магическ+шар копилк air+swimmer _lego_ радиоуправл светодиод ночник фотоподарк _бонг небесн+фонар bobblehead фотопазл подарочн подар _игр покер вертол), %w(подар+здоров) ],
}
