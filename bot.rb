...
$categories = {
  'Кафе, рестораны' => [ %w(романт+ужин чайн+церемон кафе ресторан меню _чай+_бар), %w(суши пицца техн+кухн кафел стрип) ],
  'Пицца' => [ %w(пицц) ],
  ...
  'Концерты, цирк' => [ %w(баскетбольн+матч концерт шоу цирк представлен stand-up фестивал), %w(клуб театр _дом шоу+студ доставк шоу+_рум выставк _отел представленн) ],
  'Боулинг, бильярд' => [ %w(боулинг бильярд), %w(_баз+отдых романтич+выходн проживан) ],
  ...
  'Активный отдых' => [ %w(квадроцикл гольф воздушн+шар экстрим лабиринт конн+прогулк колес+обозрен дайвинг каток катка _ката+коньк прокат+коньк) ],
  'Отдых и туризм' => [ %w(романтич+выходн круиз экскурс+тур курорт отдых туристич туризм отель туроператор travel гостиниц санатор _тур каникул Тайланд Египет Азия _Бали Эмираты проживан перелет путешеств авиаперелет Португалия hotel турбаз коттедж Вьетнам Лондон Финляндия Дубай горнолыжн), %w(_маск) ],
  'Парикмахерские' => [ %w(волос стрижк укладк ламинирован мелирован каутеризац наращиван завивк выпрямлен окрашиван биозавивк восстановлен+волос косичк), %w(ресниц мезотерап набор аксессуар маникюр+наращиван наращиван+ногт эпиляц средств+рост сыворотк шампун собак _кошк _фен фотосесси машинк+волос паркет) ],
  'Солярии' => [ %w(соляр), %w(тренажер+_зал фитнес аэроб _танц _заняти маникюр) ],
  'Уход за ногтями' => [ %w(shellac парафинотерап _ногт маникюр педикюр наращиван+гел наращиван+ногт наращиван+маникюр), %w(ресниц) ],
  'Массаж' => [ %w(массаж мануальн) ],
  'Тату салоны' => [ %w(_тату татуаж перманент+макияж) ],
  'Салоны красоты' => [ %w(мезотерап программ+груд уход+лиц салон+крас шугаринг эпиляци увеличен ревитализац макияж кожа чистк+лиц кож+лиц пилинг акне кавитац лифтинг чистк+спин), %w(spa целлюлит _кожан gillette перманент+макияж) ],
  ...
  'SPA салоны' => [ %w(spa _спа обертыван антицеллюлитн пенн+массаж), %w(машин авто спальн экскурси _спайк) ],
  'Бани и сауны' => [ %w(хаммам саун _бан джакуз), %w(_банк _бано банан бонтон баннер тренажер курорт проживан) ],
  ...
  'Фитнес центры, спорт' => [ %w(шейпинг _кунг+фу единоборств тонус+клуб теннис бокс фитнес тренажер бассейн), %w(nestle) ],
  'Йога' => [ %w(йог тибетск цигун) ],
  'Танцы' => [ %w(сальса _танц pole+dance belle+dance dance), %w(_игрушк праздн) ],
  'Уроки, занятия' => [ %w(иностран+язык лекци online+курс мини+курс экспрес+курс фотокурс автошкол _курс занят обучен учебн+центр _вожден научит научис изучен вебинар освоить осваив онлайн+курс _урок), %w(конкурс курсовая) ],
  ...
  'Ремонт и уборка в доме, дизайн' => [ %w(квартир ремонт+кварт чистк+мебел уборк диз+кварт пластик+окна _ковр переезд дизайн+проект дизайн+комн дизайн+интер), %w(_авто пылесос) ],
