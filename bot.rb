...
$categories = {
  ...
  'Солярии' => [ %w(загар+студ загар+абонемент автозагар минут+загара соляр моментальн+загар), %w(тренажер+_зал фитнес+клуб аэроб _танц _заняти маникюр солярис) ],
  ...
  'Подарки, игрушки, интересные штуки' => [ %w(лазерн+указ _магнит мишка днем+рожден день+рожден светильник жвачк+рук головоломк _слепок _слепоч монетк летающ+_рыб магическ+шар копилк air+swimmer _lego_ радиоуправл светодиод ночник фотоподарк _бонг небесн+фонар bobblehead фотопазл подарочн подар _игр покер вертол), %w(подар+здоров) ],
}
