...
URL = 'http://autokupon.ru/aggregators/info.xml'
PROVIDER = Provider.find_by_name('autokupon')

log = Logger.new(File.expand_path('../logs/autokupon.log', __FILE__))
...
log.info("Starting autokupon parser: #{Time.now}")
...
cities.each do |city|
  ...
  city_offers.each do |offer|
    ...
    if Offer.where(provider_id: PROVIDER.id, title: offer.xpath('name').text).any?
...
log.info("Finished autokupon parser. #{saved} offers added. #{Time.now}")
