...
URL = 'http://bonusprice.ru/export/'
PROVIDER = Provider.find_by_name('bonusprice')

log = Logger.new(File.expand_path('../logs/bonusprice.log', __FILE__))
...
log.info("Starting bonusprice parser: #{Time.now}")
...
#  city_offers = xml_offers.select { |offer| offer.xpath('region').text == cities[city] }
  ...
  xml_offers.map do |offer|
    if offer.xpath('region').text.strip == city.russian_name
      city_offers << offer
      nil
    else
      offer
  ...
  city_offers.each do |offer|
    ...
    offer_attributes = {
      ...
      description: Nokogiri::HTML(offer.xpath('description').text).css('body').inner_html,
...
log.info("Finished bonusprice parser. #{saved} offers added. #{Time.now}")
