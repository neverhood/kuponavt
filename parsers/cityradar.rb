...
URL = 'http://cityradar.ru/kuponator.xml'
PROVIDER = Provider.find_by_name('cityradar')
...
log = Logger.new(File.expand_path('../logs/cityradar.log', __FILE__))

xml_offers = Nokogiri::XML( open URL ).xpath('//offer')
saved = 0

log.info("Starting cityradar parser: #{Time.now}")
...
cities.each do |city|
  ...
  city_offers.each do |offer|
    ...
    binding.pry if Offer.where(provider_id: PROVIDER.id, title: offer.xpath('title').text).any?
...
log.info("Finished cityradar parser. #{saved} offers added. #{Time.now}")
