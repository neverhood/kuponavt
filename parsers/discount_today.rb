...
URL = 'http://www.discount-today.ru/xml/'
PROVIDER = Provider.find_by_name('discount-today')

log = Logger.new(File.expand_path('../logs/discount_today.log', __FILE__))
...
log.info("Starting discount_today parser: #{Time.now}")
...
#  city_offers = xml_offers.select { |offer| offer.xpath('region').text == cities[city] }
  ...
  city_offers.each do |offer|
    ...
    offer_attributes = {
      ...
      image: open( 'http://www.discount-today.ru' + offer.xpath('picture').text.gsub(' ', '%20') ),
    ...
    offer_attributes[:city_id] = City.find_by_name('sankt-peterburg').id if offer_attributes[:address] =~ /санкт/i
...
log.info("Finished discount_price parser. #{saved} offers added. #{Time.now}")
