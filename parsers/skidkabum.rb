...
URL = 'http://skidkabum.ru/rss/all'
PROVIDER = Provider.find_by_name('skidkabum')

log = Logger.new(File.expand_path('../logs/skidkabum.log', __FILE__))

cities = {
  City.find_by_name('ulyanovsk') => "Ульяновск",
  City.find_by_name('nijnii-novgorod') => "Н.Новгород",
  City.find_by_name('penza') => "Пенза",
  City.find_by_name('dmitrovgrad') => "Димитровград"
}

xml_offers = Nokogiri::XML( open URL ).xpath('//offer')
saved = 0

log.info("Starting skidkabum parser: #{Time.now}")
...
cities.keys.each do |city|
  ...
  xml_offers.map do |offer|
    if offer.xpath('region').text == cities[city]
      city_offers << offer
      nil
    else
      offer
  ...
  city_offers.each do |offer|
    ...
    if offer.xpath('picture').text == 'http://skidkabum.ru/'
      bot = Mechanize.new
      bot.get offer_attributes[:url]
      offer_attributes[:image] = open( bot.page.parser.css('#rotator img').first['src'] )
    end
...
log.info("Finished skidkabum parser. #{saved} offers added. #{Time.now}")
